...
<template>
  <div v-if="abaAtiva" class="relative h-full bg-secundario border-r border-borda-principal flex flex-col" :style="telaPequena ? { width: '100%' } : { width: largura + 'px' }">
    ...
    <div v-if="!telaPequena" class="absolute top-0 right-0 w-1 h-full cursor-ew-resize z-10 hover:bg-borda-destaque" @mousedown="iniciarRedimensionamento"></div>
  </div>
</template>
